...
import { endTimeCache, MAXIMUM_CHUNK_SIZE } from './TradingChart.constants';
import { Bar, Candle, TimestampChunk } from './TradingChart.types';
import { CandleDetails } from './hooks/useGetCandles';
...
export const hasDirectFeed = (symbolName: string) => {
  const [, quote] = symbolName.split('/');
  return [COMMON_SYMBOLS.BTC, COMMON_SYMBOLS.XUSD].includes(quote);
};
...
export const queryPairByChunks = async (
  ...
  candleLimit: number = MAXIMUM_CHUNK_SIZE,
...
export const setEndTime = (
  ...
  const key = `${baseToken}-${quoteToken}`;
  endTimeCache.set(key, timestamp);
};
...
const getFirstCandleTimestamp = async (
  ...
  return client
    ...
    .then(response => response.data?.[candleDetails.entityName] || [])
    .then(items => items[0]?.periodStartUnix || Date.now() / 1e3);
};
...
export const queryCustomPairs = async (
  ...
  return Promise.all([
    queryDirectPair(
      ...
      candleDetails,
      baseToken,
      await getTokenAddress(COMMON_SYMBOLS.XUSD),
      startTime,
      endTime,
      candleLimit,
    ),
