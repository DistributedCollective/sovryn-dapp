...
import {
  ...
  useUserSwapBorrowRateTransactionsLazyQuery,
} from '../../../../../utils/graphql/bobAave/generated';
import { dateFormat } from '../../../../../utils/helpers';
import { COLUMNS } from './AaveSwapRateModeLoanFrame.constants';
import { normalizeUserLiquidationTransactions } from './AaveSwapRateModeLoanFrame.utils';
import { useGetSwapRateModeLoan } from './hooks/useGetSwapRateModeLoan';

const pageSize = DEFAULT_HISTORY_FRAME_PAGE_SIZE;

export const AaveSwapRateModeLoanFrame: FC<PropsWithChildren> = ({
  children,
}) => {
  ...
  const { data, loading, refetch } = useGetSwapRateModeLoan(
    account,
    pageSize,
    page,
    orderOptions,
  ...
  const [getSwapBorrowRateHistory] = useUserSwapBorrowRateTransactionsLazyQuery(
    {
  ...
  const exportData = useCallback(async () => {
    const { data } = await getSwapBorrowRateHistory({
      variables: {
        skip: 0,
    ...
    <>
      <div className="flex-row items-center gap-4 mb-7 flex justify-center lg:justify-start">
        ...
        <div className="flex-row items-center ml-2 gap-4 hidden lg:inline-flex">
          <ExportCSV
            getData={exportData}
            filename="Swap borrow rate mode"
