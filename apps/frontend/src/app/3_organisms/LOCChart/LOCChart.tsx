...
export const LOCChart: FC<LOCChartProps> = ({
  ...
  useEffect(() => {
    if (userOpenTrove?.trove && !activeBar) {
  ...
  useEffect(() => {
    if (!loadingTroves && troves && !isUserOpenTrove) {
  ...
  }, [price, troves, loadingTroves, isUserOpenTrove]);
