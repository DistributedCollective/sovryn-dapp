...
export const multiSwap = async (env: CrocEnv) => {
  const ENTRY_AMOUNT = await env.token(ENTRY).normQty(1);
  ...
  // pool.swap.isBuy = true;
  // pool.swap.inBaseQty = false;
  pool.swap.qty = ENTRY_AMOUNT;
  pool.swap.limitPrice = encodeCrocPrice(PRICE_1 * (1 + SLIPPDAGE));
  ...
  console.log(
    'price',
    pool.swap.limitPrice.toString(),
    'curve',
    POOL_1_CURVE.priceRoot_.toString(),
  );
