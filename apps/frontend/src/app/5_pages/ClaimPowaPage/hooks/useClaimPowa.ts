...
export const useClaimPowa = (onComplete?: () => void) => {
  const chainId = useCurrentChain();
  const { signer, account } = useAccount();
  const { setTransactions, setIsOpen, setTitle } = useTransactionContext();

  const contract = useGetProtocolContract('merkleDistributor', RSK_CHAIN_ID);
  ...
  const claim = useCallback(async () => {
    if (!signer || chainId !== RSK_CHAIN_ID) {
      return;
    }
    ...
    setTransactions([
      {
        title: t(translations.claimPowaPage.claimTx.title),
        request: {
    ...
    setTitle(t(translations.claimPowaPage.claimTx.title));
    setIsOpen(true);
  }, [
    account,
    chainId,
