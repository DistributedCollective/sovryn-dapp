// @dev this is local fix until https://github.com/ethers-io/ethers.js/pull/2937 is merged in.
...
async function waitForSync(
  ...
  const provider = config.provider as BaseProvider;
...
export class FallbackProvider extends BaseProvider {
  ...
  async detectNetwork(): Promise<Network> {
    const networks = await Promise.all(
      this.providerConfigs.map(async (c, index) => {
        // do not crash entire provider if one of providers are down.
        try {
          return await c.provider.detectNetwork();
        } catch (error) {
          // TODO: we should ping devops somehow to notify about unresponsive provider
          console.error('[DETECT-NETWORK]', error);
          // Make failing provider less likely to be chosen again
          this.providerConfigs[index].priority = 512;
          this.providerConfigs[index].weight = 0;
          return null;
        }
      }),
    );
