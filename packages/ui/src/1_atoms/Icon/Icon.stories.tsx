...
const Template: Story<ComponentProps<typeof Icon>> = args => {
  return (
    <>
      <Icon {...args} />
      <span className="text-lg">Default Warning Icon</span>
    </>
  );
};
...
const FontAwesomeIconTemplate: Story<ComponentProps<typeof Icon>> = args => {
  return (
    <div className="pt-4">
      <Icon {...args} />
      <span className="text-lg">FontAwesome Icon</span>
    </div>
  );
};
...
const AllIconsTemplate: Story<ComponentProps<typeof Icon>> = () => {
  ...
  const allIcons = useMemo(() => {
    return icons.map((icon: IconName, key) => {
      return (
        <div
          key={key}
          className="flex items-center cursor-pointer text-black flex-col p-6 bg-sov-white rounded-md hover:bg-primary hover:text-secondary"
          onClick={() => navigator.clipboard.writeText(icon)}
        >
          <Icon key={key} size={24} icon={icon} />
          <span className="text-center py-3">{icon}</span>
        </div>
      );
    });
  }, [icons]);
  ...
  return (
    <>
      <h2 className="my-4 text-center">Click on [icon] to copy name</h2>
      <div className="grid grid-cols-4 lg:grid-cols-6 gap-4">{allIcons}</div>
    </>
  );
};
...
Basic.argTypes = {
  ...
  className: {
    control: 'text',
    description: 'The class to apply to the icon.',
  },
  dataAttribute: {
    control: 'string',
    description:
